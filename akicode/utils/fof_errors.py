'''Version of fof used to detect disparities across residuals'''
...
from n_cv_errors import n_cv, desc_dict
...
if args.on_server:
    ...
    n_feat = 3
    X, y = datasets.make_classification(n_samples=900, n_features=n_feat, n_informative=2, n_redundant=0, weights=[0.93,0.07], random_state=7)
...
ess = [
    Pipeline([("im", Imputer(missing_values=np.NAN, strategy="most_frequent", axis=0, verbose=10)),
                                 ("sc", StandardScaler()),
                   ("lasso", lr(random_state=args.random_state))]),
 #   Pipeline([("im", Imputer(missing_values=np.NAN, strategy="most_frequent", axis=0, verbose=10)),
 #                                ("sc", StandardScaler()),
...
#    },
    {
        #"gbc__n_estimators": uniform(1,500),#[100],
        #"gbc__min_samples_split": uniform(1,1000),#[150],#randint(10,200), #larger -> more bias
        #"gbc__min_samples_leaf": uniform(1,1000), #[100],#randint(50,1000)
	#    "gbc__max_depth": uniform(1,1000),#[2], # want weak (high bias) estimators, so cap max depth
        #"gbc__subsample": uniform(0,1),
        #"gbc__max_features":["auto", "sqrt", "log2", "None"],
        #"gbc__max_leaf_nodes": uniform(0,1000),
...
t0 = time()
fit_counter = 0
best_clf, m_d_outers, param_res, all_param_res, all_param_full_res, y_trues_hats, searches, best_searches, full_searches, best_full_search, all_co, best_co, all_full_co, best_full_co , fit_counter, out_splits = n_cv(X=X, y=y, 
groups=groups,
