...
#python -m pdb fof2_rnn.py -rs 3 -skgf 0 -ons 0 -nj 1 -cal3 0 -x /homedirec/user/X_aki_full_dialE_timeseries.csv.npy -y /homedirec/user/y_aki_full_dialE_timeseries.csv -g /homedirec/user/groups_aki_full_dialE_timeseries.csv -o /homedirec/user/fof_full3QFit_dialE_rnn -weight 0 -samp 0
#
if args.on_server:
    print "Running on server."
    y = pandas.read_csv(args.y_path, index_col=0) #note diff than other fof
    y = y['target'].values
	...
	feat_names = []
	n_samp = 200
	X, y = datasets.make_classification(n_samples=n_samp,
...
#y, groups = np.ravel(np.array(y)), np.array(groups)
...
def neg_log_loss(y, y_pred, labels):
    rv = -1*log_loss(y, y_pred) #so bigger is better
    return rv

grid_scoring = make_scorer(neg_log_loss, needs_proba=True, labels=sorted(np.unique(y)))
...
rnn = 1
if rnn:
    "Print only running rnn"
    ess = [
        Pipeline([("im", Imputer(missing_values=np.NAN, strategy="most_frequent", axis=0, verbose=10)),
                                     ("sc", StandardScaler()),
                       ("rnn", KerasClassifier(build_fn=model, num_steps=num_steps, num_feat=num_feat, verbose=0, epochs=10)) ]),
