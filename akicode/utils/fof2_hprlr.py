'''Version of fof used for highly penalized randomized LR'''

from sklearn import datasets
from sklearn.utils import shuffle
#
from sklearn.linear_model import LogisticRegression as lr
from sklearn.linear_model import Lasso, Ridge, RandomizedLogisticRegression
...
if args.on_server:
    ...
    n_feat = 50
    X, y = datasets.make_classification(n_samples=9000, n_features=n_feat, n_informative=20, n_redundant=0, weights=[0.5,0.5], 
random_state=7)
    ...
    pandas.DataFrame(X).to_csv('X_test.csv')
    pandas.DataFrame(y).to_csv('y_test.csv', header=False)
        ...
        Pipeline([("im", Imputer(missing_values=np.NAN, strategy="most_frequent", axis=0, verbose=10)),
		       ...
		       ("fs", RandomizedLogisticRegression(
						    ...
						    random_state=1, n_jobs = 1, 
                                                    sample_fraction=0.75, n_resampling=50) ),
                       ("lr1", lr(n_jobs=args.n_jobs, C=1, class_weight='balanced', #C NOT PEN!!! 
